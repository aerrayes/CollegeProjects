...
OR2::OR2(const GraphicsInfo &r_GfxInfo, int r_FanOut):Gate(2, r_FanOut,pair<int,int>(59,16),
	pair<int,int>(0,9),pair<int,int>(0,24))
{
...
void OR2::Draw(ApplicationManager* pManager)
{
	if(!Mark)
	{
...
void OR2::change_graphic(int x,int y)
{
	m_GfxInfo.x1 =x;
	m_GfxInfo.y1 = y;
...
void OR2::DrawConnections(ApplicationManager*A)
{
...
void OR2::SaveComponent(int i,ApplicationManager *pManager)
{
...
CompInfo OR2::getCompSave()
{
	return COMPS;
}

string OR2::getComp_Type()
 {
	/* COMPS.compType;*/
	 return "OR2";
}
