...
Nand2::Nand2(const GraphicsInfo &r_GfxInfo, int r_FanOut):Gate(2, r_FanOut,pair<int,int>(59,16),
	pair<int,int>(0,8),pair<int,int>(0,24))
{
...
void Nand2::Draw(ApplicationManager* pManager)
{
	if(!Mark)
	{
...
void Nand2::change_graphic(int x,int y)
{
	m_GfxInfo.x1 =x;
	m_GfxInfo.y1 = y;
...
void Nand2::DrawConnections(ApplicationManager*A)
{
...
void Nand2::SaveComponent(int i,ApplicationManager *pManager)
{
...
CompInfo Nand2::getCompSave()
{
	return COMPS;
}

string Nand2::getComp_Type()
 {
	/* COMPS.compType;*/
	 return "NAND2";
}
