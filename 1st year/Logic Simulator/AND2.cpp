...
#include <utility>
AND2::AND2(const GraphicsInfo &r_GfxInfo, int r_FanOut):Gate(2, r_FanOut,pair<int,int>(60,20),
	pair<int,int>(0,10),pair<int,int>(0,29))
{
...
void AND2::Draw(ApplicationManager* pManager)
{
	if(!Mark)
	{
...
void AND2::change_graphic(int x,int y)
{
	m_GfxInfo.x1 =x;
	m_GfxInfo.y1 = y;
...
void AND2::DrawConnections(ApplicationManager*A)
{
...
void AND2::SaveComponent(int i,ApplicationManager *pManager)
{
...
CompInfo AND2::getCompSave()
{
	return COMPS;
}

string AND2::getComp_Type()
 {
	 return "AND2";
}
