...
void Nor3::Operate()
{
	if(m_InputPins[0].getStatus()==LOW && m_InputPins[1].getStatus()==LOW&&m_InputPins[3].getStatus()==LOW )
		m_OutputPin.setStatus(HIGH);
	else m_OutputPin.setStatus(LOW);
}
Nor3:: Nor3(const GraphicsInfo &r_GfxInfo, int r_FanOut):Gate(3, r_FanOut,pair<int,int>(59,15),
	pair<int,int>(0,8),pair<int,int>(0,15),pair<int,int>(0,22))
{
...
void Nor3::Draw(ApplicationManager* pManager)
{
	if(!Mark)
	{
...
void Nor3::change_graphic(int x,int y)
{
	m_GfxInfo.x1 =x;
	m_GfxInfo.y1 = y;
...
void Nor3::DrawConnections(ApplicationManager*A)
{
...
void Nor3::SaveComponent(int i,ApplicationManager *pManager)
{
...
CompInfo Nor3::getCompSave()
{
	return COMPS;
}

string Nor3::getComp_Type()
 {
	/* COMPS.compType;*/
	 return "NOR3";
}
